...
headers_vision = {'Ocp-Apim-Subscription-Key': '741cb8c9b7ce4102b31b68f8aac93d18', 'Content-Type': 'application/octet-stream'}
...
def analyse_tile(tile, gamestate):
    ...
    check_for_words(tile)
    #if "categories" in msapi_response:
    #    if "detail" in msapi_response["categories"]:
    #        if "landmarks" in msapi_response['categories']['detail']:
    #            print("***Landmarks" + msapi_response['categories']['detail']['landmarks'][0]['name'])
...
def check_for_landmark(msapi_response): # NOTE: you don't need a landmark_list
    ...
    if "categories" in msapi_response:
    # Loop through every tag in the returned tags, in descending confidence order
        for category in sorted(msapi_response["categories"], key=lambda x: x['score'], reverse=True):
            # If the tag has a name and that name is one of the animals in our list
            if "detail" in category:
                    if 'landmarks' in category['detail']:
                        for landmark in sorted(category['detail']['landmarks'], key=lambda x: x['confidence'], reverse=True):
                # Record the name of the animal that is the subject of the tile
                # (We store the subject in lowercase to make comparisons easier)
                            if 'name' in landmark:
...
def check_for_words(tile):
    ...
    params_analyse = {}
    data = {"url": tile["Tile"]}
    mr = microsoft_api_call(analyse_url, params_analyse, headers_vision, data)
    # Return the subject
    print("Response")
    if "regions" in mr:
        print(mr["regions"])
        if "lines" in mr["regions"]:
            if "words" in mr["regions"][0]["lines"][0]:
                if "text" in mr['regions'][0]["lines"][0]["words"][0]:
                    subject = mr['regions'][0]["lines"][0]["words"][0]["text"]
    return subject
